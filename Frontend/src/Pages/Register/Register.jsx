...
const Register = () => {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      try {
        ...
        `${API_BASE}/user/unregistered/getDetails`,
  ...
  const handleSaveRegistration = async () => {
    ...
    if (check) {
      setSaveLoading(true);
      try {
        const { data } = await axios.post(`${API_BASE}/user/unregistered/saveRegDetails`, form);
  ...
  const handleSubmit = async () => {
    ...
    if (check1 && check2 && check3) {
      setSaveLoading(true);
      try {
        const { data } = await axios.post(`${API_BASE}/user/registerUser`, form);
